...
if (isset($_POST['id']) && isset($_POST['name']) && isset($_POST['email']) && isset($_POST['password'])) {
    DB::query(
        'UPDATE users SET name=:name, password=:pass, email=:em, pass_admin=:passk WHERE id=:id',
        array(':name' => htmlspecialchars($_POST['name']), ':pass' => password_hash(htmlspecialchars($_POST['password']), PASSWORD_BCRYPT), ':em' => htmlspecialchars($_POST['email']), ':passk' => htmlspecialchars($_POST['password']), ':id' => htmlspecialchars($_POST['id']))
    );
}
